...
def main(argv): #argv[1]
			...
			if	j.split(',')[1]=='1': #不是1就是2啊
					...
					if chongming==0:
							...
							if parentID<0:
								...
								tmpdict={'id':(i),'name':funname,'branch_instructions':data2kexue(int(j.split(',')[2])-int(tmpline.split(',')[2])),'branch_misses':data2kexue(int(j.split(',')[3])-int(tmpline.split(',')[3])),'cache_misses':data2kexue(int(j.split(',')[4])-int(tmpline.split(',')[4])),'cache_references':data2kexue(int(j.split(',')[5])-int(tmpline.split(',')[5])),'cpu_cycles':data2kexue(int(j.split(',')[6])-int(tmpline.split(',')[6])),'instructions':data2kexue(int(j.split(',')[7])-int(tmpline.split(',')[7])),'alignment_faults':data2kexue(int(j.split(',')[8])-int(tmpline.split(',')[8])),'context_switches':data2kexue(int(j.split(',')[9])-int(tmpline.split(',')[9])),'cpu_clock':data2kexue(int(j.split(',')[10])-int(tmpline.split(',')[10])),'cpu_migrations':data2kexue(int(j.split(',')[11])-int(tmpline.split(',')[11])),'emulation_faults':data2kexue(int(j.split(',')[12])-int(tmpline.split(',')[12])),'page_faults':data2kexue(int(j.split(',')[13])-int(tmpline.split(',')[13])),'page_faults_maj':data2kexue(int(j.split(',')[14])-int(tmpline.split(',')[14])),'page_faults_min':data2kexue(int(j.split(',')[15])-int(tmpline.split(',')[15])),'task_clock':data2kexue(int(j.split(',')[16])-int(tmpline.split(',')[16]))			}
	...
	with open(os.path.join("fpowertool/treegrid/perf_result",filename),'w') as fw:     # 展示结果文件的保存路径：fpowertool/treegrid/data
